...
<Dialog.Portal {...portalProps}>
  ...
  <DialogPrimitive.Content
    ...
    class={cn(
      ...
      'fixed top-[50%] left-[50%] z-50 grid w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 border bg-background p-6 shadow-lg duration-150 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:animate-in data-[state=open]:fade-in-0 sm:rounded-lg',
      className
    )}
    {...restProps}>
    {@render children?.()}
